...
15. 在backend 文件夹下 针对于python项目flask架构 优化项目结构，剔除不必要的文件和文件夹，比如 一些测试文件，一些备份文件，一些旧版本的文件等等，保留task文件夹内容，优化项目结构，让项目更加清晰，易维护。
16. 我需要在backend项目中，task文件夹下，重新开发一个py脚本，用于自动获取相关雪场（北大湖、可可托海、禾木、松花湖）的天气数据，每天4次自动更新，将其添加到对应的数据库中，并且根据相关雪场的天气数据，在control/routes.py文件中生成对应的API接口，用于前端页面调用，更新前端页面的天气内容
